...
static int ggml_cuda_try_fuse(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph, int i) {
    ...
    if (ggml_can_fuse(cgraph, i,
        ...
        // x is in the supported whitelist and every chain intermediate shares x's
        // type, since the kernel templates over {float, half, nv_bfloat16}. Broadcast
        // operands a and inv_b are bound as const float * in launch_snake, so they
        // must be F32 regardless of x's precision.
