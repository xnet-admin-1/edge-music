...
static bool qw3lm_load(Qwen3LM * m, const char * gguf_path, int max_seq_len, int n_kv_sets) {
    *m = {};
    ...
    // embed_tokens uses ggml_get_rows (unsupported on Vulkan) — load on CPU.
    // Transformer layers + final_norm on GPU for fast matmul.
    WeightCtx embed_wctx;
    wctx_init(&embed_wctx, 1);
    m->embed_tokens = gf_load_tensor(&embed_wctx, gf, "model.embed_tokens.weight");
    wctx_alloc(&embed_wctx, m->cpu_backend);

    int n_tensors = 1 + c.n_layers * 11;  // final_norm + layers
    ...
    wctx_alloc(&m->wctx, m->cpu_backend);
    gf_close(&gf);
